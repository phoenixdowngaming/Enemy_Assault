...
[_pow,["<t size='1.5' shadow='2' color='#2EFEF7'>Rescue Hostage</t>", "call ghst_fnc_hostjoin", [], 6, true, true, "","(alive _target) and (_target getVariable 'notrescued')"]] remoteExec ["fnc_ghst_addaction",0,true];
...
_trig2act = format ["[%1] joinsilent grpNull; ['%2','succeeded'] call BIS_fnc_taskSetState; if (vehicle %1 != %1) then {unassignVehicle (%1); (%1) action ['EJECT', vehicle %1]; [%1] allowGetin false; dostop %1; %1 setCaptive true;}; [playableunits,5000,100] remoteExec ['fnc_ghst_addscore'];", _pow, _tsk];
...
[_pow,_tsk,_trg2] spawn {
	...
	while {true} do {
		...
		if (!(triggerActivated _trg2) and !(alive _pow)) exitwith {[_tsk,"failed"] call BIS_fnc_taskSetState; [playableunits,0,-200] remoteExec ["fnc_ghst_addscore"]; deletevehicle _pow; deleteVehicle _trg2;};
		sleep 5;
	};
};
