...
[_intelobj,_tsk] spawn {
	...
	while {true} do {
		...
		if (!(_tsk call BIS_fnc_taskCompleted) and (isnull _intelobj)) exitwith {[_tsk,"failed"] call BIS_fnc_taskSetState; [playableunits,0,-200] remoteExec ["fnc_ghst_addscore"];};
		sleep 5;
	};
};