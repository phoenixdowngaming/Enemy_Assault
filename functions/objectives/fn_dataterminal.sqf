...
_taskdesc = format ["Search the buildings in %2 for %1. Once you have found %1 disable the Link.", _veh_name,_locselname];
...
[_terminalobj,["<t size='1.5' shadow='2' color='#2EFEF7'>Disable Terminal Link</t>", "call ghst_fnc_disableterminal", _tsk, 6, true, true, "","(_target getVariable 'notdisabled')"]] remoteExec ["fnc_ghst_addaction",0,true];
//Animate terminal
...
[_terminalobj,_tsk] spawn {
	...
	while {true} do {
		...
		if (!(_tsk call BIS_fnc_taskCompleted) and (isnull _terminalobj)) exitwith {[_tsk,"failed"] call BIS_fnc_taskSetState; [playableunits,0,-200] remoteExec ["fnc_ghst_addscore"];};
		sleep 5;
	};
};